package com.auroral.camera.view;
...
import com.auroral.camera.base.ICameraView;
import com.auroral.camera.listener.CameraListener;
import com.auroral.camera.listener.CaptureListener;
import com.auroral.camera.listener.ClickListener;
import com.auroral.camera.listener.ErrorListener;
import com.auroral.camera.listener.TypeListener;
import com.auroral.camera.state.CameraMachine;
import com.auroral.camera.util.FileUtils;
import com.auroral.camera.util.ScreenUtils;

import java.io.IOException;

public class CameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, ICameraView {
    ...
    //回调监听
    private CameraListener mCameraLisenter;
    ...
    public CameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        //Init AttributeSet
        iconSrc = R.drawable.came_camera_bg;
        iconLeft = R.drawable.came_back_bg;
        iconRight = R.drawable.came_photo_bg;
        duration = 10 * 1000;// 设置默认为10s
        initData();
        initView();
    }
    ...
    private void initView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.came_camera_view, this);
    ...
    public void setCameraLisenter(CameraListener mCameraLisenter) {
        this.mCameraLisenter = mCameraLisenter;
    }
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                mVideoView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
                machine.start(mVideoView.getHolder(), screenProp);
                if (mCameraLisenter != null) {
                    mCameraLisenter.recordSuccess(videoUrl, firstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
    ...
    public void hideRightIcon() {
        mCaptureLayout.setRightIconSrc(0);
    }

    private void setFlashRes() {
        switch (type_flash) {
            case TYPE_FLASH_AUTO:
                mFlashLamp.setImageResource(R.drawable.came_flash_auto_bg);
                machine.flash(Camera.Parameters.FLASH_MODE_AUTO);
                break;
            case TYPE_FLASH_ON:
                mFlashLamp.setImageResource(R.drawable.came_flash_on_bg);
                machine.flash(Camera.Parameters.FLASH_MODE_TORCH);
                break;
            case TYPE_FLASH_OFF:
                mFlashLamp.setImageResource(R.drawable.came_flash_off_bg);
